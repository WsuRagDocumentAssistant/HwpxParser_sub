#================================================
# parser/table/table_analyzer.py
#================================================
...
from hwpx_document.table.table_data.table_analysis import TableValidation
...
class TableAnalyzer:
    ...
    def analyze(
        ...
        if context is not None:
            cls._validate_border_fill_refs(table, validation, context)
    ...
    @classmethod
    def _validate_border_fill_refs(
        cls,
        table: Table,
        validation: TableValidation,
        ...
        """
        역할: 표와 셀의 borderFillIDRef가 header.xml의 borderFill에 존재하는지 검증한다.
        입력 데이터: table(검증 대상 Table), validation(갱신할 TableValidation), context(header 조회용 ParserContext).
        출력 데이터: 반환값은 없고, 누락된 borderFill 참조 플래그와 이슈가 갱신된다.
        """
        table_ref = table.border_fill_id_ref

        if table_ref is not None and context.get_border_fill_raw(table_ref) is None:
            validation.has_missing_border_fill_ref = True

            cls._add_issue(
                validation,
                code="MISSING_TABLE_BORDER_FILL_REF",
                message="표의 borderFillIDRef가 header.xml의 hh:borderFill에 존재하지 않습니다.",
                severity="ERROR",
                details={
                    "border_fill_id_ref": table_ref,
                    "target": "table",
                },
            )

        for cell in table.cells:
            cell_ref = cell.border_fill_id_ref

            if cell_ref is None:
                continue

            if context.get_border_fill_raw(cell_ref) is not None:
                continue
            ...
            cls._add_issue(
                validation,
                code="MISSING_CELL_BORDER_FILL_REF",
                message="셀의 borderFillIDRef가 header.xml의 hh:borderFill에 존재하지 않습니다.",
                severity="ERROR",
                cell_id=cell.cell_id,
                row_index=cell.row_addr,
                col_index=cell.col_addr,
                details={
                    "border_fill_id_ref": cell_ref,
                    "target": "cell",
                },
            )
